// ESP32 doesn't need avr/pgmspace.h, use const instead
//#include <EEPROM.h>
...
// ESP32: Use const instead of PROGMEM (ESP32 has enough RAM)
...
void GravityDo::update()
{
	...
	for (uint8_t i = 0; i < ARRAYLENGTH; i++)
	{
		...
		analogBuffer[i] = analogReadESP32(this->_pin);
		delay(10);
	}

	// ESP32: Convert using Arduino-compatible 1024.0 divisor
	// Note: analogReadESP32 already converts ESP32 12-bit to Arduino 10-bit
	...
	// ESP32: Direct array access (no PROGMEM needed)
	int tempIndex = (int)(this->_temperature + 0.5);
	if (tempIndex < 0) tempIndex = 0;
	if (tempIndex > 40) tempIndex = 40;
	this->_doValue = SaturationValueTab[tempIndex] * this->_averageVoltage / this->_saturationDoVoltage;  //calculate the do value, doValue = Voltage / this->_saturationDoVoltage * SaturationDoValue(with temperature compensation)
}
